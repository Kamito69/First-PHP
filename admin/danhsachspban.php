            ...
            <?php							
				...
				if(mysqli_num_rows($hoadon_data) > 0)
				{
					foreach ($hoadon_data as $hoadon_view) 
					{
						...
						$cthd_data = getCTHDtheoHDS("chitiethd", $hoadon_id);
                        $chitiethoadon = mysqli_fetch_array($cthd_data);
						if(mysqli_num_rows($cthd_data) > 0)
						{
							foreach ($cthd_data as $cthd_views) 
							{
                                ...
                                <div class="col-md-2"><br>        
                                    <a class="btn btn-primary" type="submit" href="addbaohanh.php?id=<?= $cthd_views['id'] ?>">Lập Bảo hành</a>
                                </div>
                                <?php
                                $prod_id = $cthd_views['MaSP'];
						        $get_product = getByID("products", $prod_id);
                                $get_baohanh = getBycthd('baohanh', $cthd_views['id']);
